...
const fetchViaRpc = async (
  ...
  try {
    ...
    if (
      ...
      auth &&
      rateLimitResetCredits?.credits === undefined
    ) {
      try {
    ...
    return {
      error: null,
      readings: codexQuotaReadings(rateLimits, updatedAt),
      provider: 'codex',
      rateLimitResetCredits:
...
const readCodexIdentity = async (options: FetchCodexQuotaOptions) => {
  try {
    return parseCodexAccountIdentity(
      await readFile(path.join(getCodexHomePath(options), 'auth.json'), 'utf8'),
    );
  } catch {
    return null;
  }
...
export const fetchCodexQuota = async (
  ...
  const identityBefore = await readCodexIdentity(options);
  const auth = await readCodexBackendAuth(options);
  const snapshot = await fetchViaRpc(options, auth);
  // Never attribute a sample if the device switched logins while it was read.
  const identityAfter = await readCodexIdentity(options);
  return {
    ...snapshot,
    identity:
      identityBefore?.externalAccountId === identityAfter?.externalAccountId ? identityAfter : null,
  };
};
