...
Neon has since stopped offering `pg_search` to new projects. Retirement timelines for existing
projects follow the notice Neon sends for each project. LobeHub users who receive such a notice
should complete
[Migrate from pg\_search on Neon](/docs/self-hosting/advanced/neon-pg-search-migration) before the
deadline in that notice.
