...
- `JWKS_KEY`: the RS256 key set LobeHub signs internal JWTs with. Every deployment must use its own key, and it stays in the `lobe` container.
- `JWKS_PUBLIC_KEY`: the same key without its private fields. Only this value is passed to the gateway, which verifies browser sessions with it.

```sh
# GATEWAY_SERVICE_TOKEN
openssl rand -hex 32
# JWKS_KEY and JWKS_PUBLIC_KEY (paste both lines into .env)
...
<Steps>
  ...
  Then add `JWKS_PUBLIC_KEY`, the public half of your existing `JWKS_KEY`, by running this in the directory that holds `.env` and pasting its output:
  ...
  If `JWKS_KEY` in `.env` is still the value shipped in the old `.env.example` (its `kid` is `6823046760c5d460`), anyone can forge tokens your deployment accepts. Generate a new key pair with the command in [Necessary Configuration](#necessary-configuration) and replace both `JWKS_KEY` and `JWKS_PUBLIC_KEY`. Desktop and CLI clients may need to sign in again afterwards.
...
`rustfs-init` also applies the bucket CORS rule that browser uploads need. If uploads fail in the browser with a network error, re-run it with `docker compose up -d --force-recreate rustfs-init`.
