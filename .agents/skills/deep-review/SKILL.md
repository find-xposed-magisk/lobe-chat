...
Multi-dimensional code review built on independent subagents. Review breadth comes from parallel
...
Both modes read the same files. Light mode reads the full `Quick checklist` section, including
nested example subsections; deep-mode agents read the full dimension file plus only the rule
sources and routed references applicable to the touched surface.
